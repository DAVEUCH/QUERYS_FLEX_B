...
LEFT JOIN eUsuarioEmpresa ue ON ue.IDUsuario = usu.PKID
LEFT JOIN UsuarioActividad uac ON uac.IDUsuario = usu.PKID
LEFT JOIN Empresa em ON em.PKID = ue.IDEmpresa
...
WHERE 
	...
	--AND em.Nombre='GCI'
	SELECT*FROM EMPRESA
	SELECT TOP 9000*FROM UsuarioActividad
...
select*from Usuario WHERE IDUsuario NOT LIKE 'XX%'
...
SELECT TOP 5 IDPerfilUsuario FROM Usuario;
SELECT TOP 5 ID FROM ePerfilUsuario;
SELECT TOP 5 * FROM ePerfilUsuario;
...
) epu ON TRY_CAST(epu.ID AS INT) = usu.IDPerfilUsuario
LEFT JOIN eUsuarioEmpresa ue ON ue.[Usuarios.IDUsuario] = usu.IDUsuario
LEFT JOIN UsuarioActividad uac ON uac.IDUsuario = usu.IDUsuario
LEFT JOIN Empresa em ON em.PKID = ue.IDEmpresa
LEFT JOIN Actividad ac ON ac.PKID = uac.IDActividad
WHERE 
    usu.IDUsuario NOT LIKE 'XX%'
    AND usu.TipoUsuario = 'PERFIL'
